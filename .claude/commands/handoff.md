...
3. **Update `docs/STATE.md`:**
   - Solo work → refresh the SOLO-mode `Now / Next (immediate action) / Uncommitted
     work` and the `Updated`/`branch`/`session` line (the full queue stays in the map).
...
4. **Do NOT** promote-and-drop lasting items in a rush — if something belongs in an
   ADR/glossary/architecture, note it in `Next` so it's captured next session.
